...
<head>
    ...
    <!-- Meta Tags SEO -->
    <meta name="description" content="Jelajahi kumpulan esai, jurnal, berita, dan catatan literasi alternatif dari para kontributor dan pegiat Kanca Tegal.">
...
<body>
    ...
    <main class="articles-page-container">
        ...
        <!-- Page Header -->
        <div class="articles-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; margin-bottom: 3rem;">
            <div>
            ...
            @auth
                @if (Auth::user()->can_upload_artikel)
                    <a href="{{ url('/upload-artikel') }}" class="btn-signin" style="background-color: var(--accent-blue); display: inline-flex; align-items: center; gap: 8px; text-decoration: none; text-transform: uppercase; font-size: 0.85rem; padding: 0.7rem 1.5rem;">
            ...
            <div class="articles-grid">
                ...
                @empty
                    <div style="grid-column: span 3; text-align: center; padding: 4rem 0; color: var(--text-muted);">
                        <i class="far fa-file-alt" style="font-size: 3rem; margin-bottom: 1rem; display: block; opacity: 0.5;"></i>
                ...
                @forelse($latest as $art)
                    ...
                    <article class="latest-article-card">
                        <div class="latest-card-img-container">
                            <img src="{{ filter_var($art->foto_utama, FILTER_VALIDATE_URL) ? $art->foto_utama : asset($art->foto_utama) }}" alt="{{ $art->judul }}" class="latest-card-img" style="height: 200px; object-fit: cover; width: 100%;">
                        </div>
                        <div class="badge-row">
                        ...
                        <div class="latest-card-footer">
                            ...
                            <button class="btn-action-arrow dynamic-art-link"
                               data-title="{{ $art->judul }}" 
                               data-uploader="{{ $art->nama_uploader }}" 
                               data-date="{{ \Carbon\Carbon::parse($art->tanggal_upload)->format('d M Y') }}" 
                               data-content="{{ $art->isi }}" 
